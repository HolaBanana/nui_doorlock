...
Citizen.CreateThread(function()
	while not esxLoaded do Wait(1000) end
...
AddEventHandler('nui_doorlock:newDoorSetup', function(args)
	...
	if doorType == 'door' or doorType == 'sliding' or doorType == 'garage' then
		...
		TriggerServerEvent('nui_doorlock:newDoorCreate', arg.configname, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, false, doorname, arg.steamIds)
	...
	elseif doorType == 'double' or doorType == 'doublesliding' then
		...
		if arg then doorname = arg.doorname end
		TriggerServerEvent('nui_doorlock:newDoorCreate', arg.configname, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, true, doorname, arg.steamIds)
		print('Successfully sent door data to the server')
		arg = nil
	end
end)
