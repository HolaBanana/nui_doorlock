...
function IsAuthorized(xPlayer, doorID, usedLockpick)
	...
	if doorID.steam and doorID.steam[xPlayer.identifier] then
		return true
	end
	...
	if xPlayer.getGroup() ~= 'user' then
		print(xPlayer.getGroup()..' '..xPlayer.name..' opened a door using admin privileges')
		return true
	end
	return false
end
...
AddEventHandler('nui_doorlock:newDoorCreate', function(config, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, doubleDoor, doorname, steamIds)
	...
	if xPlayer.getGroup() == 'user' then print(xPlayer.name.. 'attempted to create a new door but does not have permission') return end
